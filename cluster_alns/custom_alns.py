...
from cluster_alns.cvrp.utils import update_neighbor_graph as cvrp_update_neighbor_graph
from cluster_alns.tsp.utils import ai4_update_neighbor_graph
from cluster_alns.tsp.utils import update_neighbor_graph as tsp_update_neighbor_graph
...
class ALNS:
    ...
    def iterate(
        ...
        **kwargs
        ...
        while not stop(self._rnd_state, best, curr):
            ...
            # CUSTOM ADDED (to allow for graph updates):
            if self.problem_type == "ai4tsp":
                best.graph = curr.graph = ai4_update_neighbor_graph(
                    cand, cand.routes, cand.objective()
                )
            elif self.problem_type == "cvrp":
                best.graph = curr.graph = cvrp_update_neighbor_graph(
                    cand, cand.routes, cand.objective()
                )
            else:
                best.graph = curr.graph = tsp_update_neighbor_graph(
                    cand, cand.routes, cand.objective()
                )
            # elif "mtsp" in curr.problem_instance:
            #     best.graph = curr.graph = mtsp_update_neighbor_graph(
            #         cand, cand.routes, cand.objective()
            #     )
    ...
    def _determine_outcome(
        ...
        if cand.objective(best=True) < best.objective(
            best=True
        ):  # candidate is new best
            # logger.info(f"New best with objective {cand.objective(best=True):.2f}.")
            # print('new best', cand.objective(best=True))
            # print("new best: ", cand.objective())
