...
class AI4TSPState:
    def __init__(
        ...
        adj: list[list[float]],
        seed: int,
        k_optimal_instance: int = 0,
    ):
        self.nodes = nodes
        self.routes = init_route
        ...
        self.k_optimal_instance = k_optimal_instance
    ...
    def check_solution(self, sol: list[int]) -> tuple[float, float, float, bool]:
        assert (
            len(sol) == len(self.x) + 1
        ), f"len(sol) = {len(sol)}, n_nodes+1 = {len(self.x) + 1}"
        assert len(sol) == len(set(sol)) + 1
        tour_time, rewards, pen, feas = tour_check(
            sol, self.x, self.adj, self.maxT_pen, self.tw_pen, len(self.nodes)
        )
    ...
    def objective(self, best=False) -> float:
        ...
        unvisited_nodes = [x for x in self.nodes if x not in self.routes]
        evaluation_tour = self.routes + unvisited_nodes
