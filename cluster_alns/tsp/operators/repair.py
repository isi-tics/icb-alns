...
def get_regret_single_insertion(args):
    ...
    for i in range(len(route) + 1):
        ...
        if i == 0:
            cost_difference = (
                distance_matrix_data[updated_route[-1], updated_route[i]]
                + distance_matrix_data[updated_route[0], updated_route[1]]
                - distance_matrix_data[updated_route[-1], updated_route[1]]
            )
        elif i == len(route):
            cost_difference = (
                distance_matrix_data[updated_route[i - 1], updated_route[i]]
                + distance_matrix_data[updated_route[i], updated_route[0]]
                - distance_matrix_data[updated_route[i - 1], updated_route[0]]
            )
        else:
            cost_difference = (
                distance_matrix_data[updated_route[i - 1], updated_route[i]]
                + distance_matrix_data[updated_route[i], updated_route[i + 1]]
                - distance_matrix_data[updated_route[i - 1], updated_route[i + 1]]
            )
...
def regret_insertion(current, random_state, prob=1.5, **kwargs):
    visited_customers = set(current.routes)
    ...
    while unvisited_customers:
        ...
        insertion_option = 0
        while (
            random.random() < 1 / prob and insertion_option < len(insertion_options) - 1
        ):
            insertion_option += 1
        repaired.routes = list(sorted(insertion_options, reverse=True)[insertion_option])

        visited_customers = set([customer for customer in repaired.routes])
        unvisited_customers = all_customers - set(visited_customers)
    return repaired
...
def multi_processing_regret_insertion(current, random_state, prob=1.5, **kwargs):
    ...
    while unvisited_customers:
        args_list = [
            (customer, repaired.routes, current.dist_matrix_data)
            for customer in unvisited_customers
        ]
